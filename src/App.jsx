...
export default function App() {
  ...
  const [gcalClientIdLocal, setGcalClientIdLocal] = useLocalStorage("cml_gcal_client_id", "");
  const gcalClientId = import.meta.env.VITE_GCAL_CLIENT_ID || gcalClientIdLocal;
        ...
        <div style={{ position: "absolute", top: 16, right: 24, zIndex: 10 }}>
          ...
          {showApiKey && (
            <div style={{
              ...
              {/* Google Calendar — only show if no env variable */}
              {!import.meta.env.VITE_GCAL_CLIENT_ID && (
                <>
                  ...
                  <input
                    type="password"
                    style={{ ...inputStyle, fontSize: 12, marginBottom: 4 }}
                    value={gcalClientIdLocal}
                    onChange={e => setGcalClientIdLocal(e.target.value)}
